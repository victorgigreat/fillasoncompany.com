...
<head>
    ...
    <style>
        ...
        /* New styles for expense and report sections */
        .date-range-picker {
            display: flex;
            gap: 10px;
            align-items: center;
    ...
    <div class="container mt-4 mb-5">
        ...
        <div id="reports-section" class="content-section">
            ...
            <div id="report-results" class="d-none">
                <div class="card mb-4">
                    <div class="card-header bg-white">
                        <i class="fas fa-list me-2"></i>Report Summary
                    </div>
                    <div class="card-body summary-card">
                        <div class="row">
                            <div class="col-md-3 summary-item">
                                <div class="summary-label">Total Sales (₦)</div>
                                <div id="report-total-sales">--</div>
                            </div>
                            <div class="col-md-3 summary-item">
                                <div class="summary-label">Total Profit (₦)</div>
                                <div id="report-total-profit">--</div>
                            </div>
                            <div class="col-md-3 summary-item">
                                <div class="summary-label">Total Expenses (₦)</div>
                                <div id="report-total-expenses">--</div>
                            </div>
                            <div class="col-md-3 summary-item">
                                <div class="summary-label">Total Returns</div>
                                <div id="report-total-returns">--</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card mb-4">
                    ...
                    <div class="card-body p-0">
                        <div id="report-sales" class="table-responsive">
                            <p class="text-center my-5">No sales data available</p>
                        </div>
                    </div>
                </div>
                <div class="card mb-4">
                    <div class="card-header bg-white">
                        <i class="fas fa-money-bill-wave me-2"></i>Expenses Details
                    </div>
                    <div class="card-body p-0">
                        <div id="report-expenses" class="table-responsive">
                            <p class="text-center my-5">No expenses data available</p>
                        </div>
                    </div>
                </div>
                <div class="card mb-4">
                    <div class="card-header bg-white">
                        <i class="fas fa-undo me-2"></i>Returns Details
                    </div>
                    <div class="card-body p-0">
                        <div id="report-returns" class="table-responsive">
                            <p class="text-center my-5">No returns data available</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            if (data.expenses.length === 0) {
                ...
                data.expenses.forEach(expense => {
                    html += `<tr>
                        ...
                        <td><button class="btn btn-sm btn-danger delete-expense" data-id="${expense.id}"><i class="fas fa-trash"></i></button></td>
                    </tr>`;
                });
            }
    ...
    $.ajax({
        ...
        success: function(data) {
                    ...
                    html += `<tr>
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                ...
                                <button class="btn btn-warning edit-threshold" 
                                    data-id="${product.id}" 
                                    data-threshold="${product.low_stock_threshold || 200}"
            ...
            $('.edit-threshold').click(function() {
                const productId = $(this).data('id');
                const currentThreshold = $(this).data('threshold');
                
                const newThreshold = prompt('Enter new low stock threshold (minimum 1):', currentThreshold);
                if (newThreshold && !isNaN(newThreshold) && parseInt(newThreshold) > 0) {
                    updateThreshold(productId, parseInt(newThreshold));
...
function updateThreshold(productId, threshold) {
    $.ajax({
        url: 'api/update_threshold.php',
        type: 'POST',
        data: { 
            product_id: productId, 
            threshold: threshold 
        ...
        success: function(response) {
            if (response.success) {
                ...
                showAlert(response.error || 'Failed to update threshold', 'danger');
            }
        },
        error: function(xhr) {
            showAlert('Error updating threshold: ' + xhr.responseText, 'danger');
        }
    });
}
